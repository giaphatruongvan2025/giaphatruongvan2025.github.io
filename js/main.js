...
async function loadData() {
    try {
        const res = await fetch("data/genealogy.json");
        const data = await res.json();
        localStorage.setItem("giaPhaData", JSON.stringify(data));
        return data;
    } catch (err) {
        console.warn("Không thể load JSON, sử dụng localStorage:", err);
...
async function initTree() {
    ...
    function showInfo(p) {
        ...
        if(!p){ box.innerHTML="<p>Không tìm thấy!</p>"; return; }
        box.innerHTML = `
    ...
    function drawTree(centerName=null){
        ...
        centerNode.children = people.filter(p=>p.parent===center.fullName).map(c=>({...c, children:[]}));

        let treeData;
        if(parent){ parent.children=[centerNode]; treeData={name:"Gia phả", children:[parent]}; }
        else treeData={name:"Gia phả", children:[centerNode]};
    ...
    function renderTree(treeData){
        ...
        d3.tree().size([height,width-200])(root);

        // lines
        svg.append("g").selectAll("path")
          .data(root.links()).join("path")
          .attr("fill","none").attr("stroke","#8B0000").attr("stroke-width",2)
        ...
        node.append("rect")
          .attr("width",210).attr("height",60).attr("x",-105).attr("y",-30).attr("rx",10)
          .attr("fill","#fff4cc").attr("stroke","#8B0000");
        ...
        node.append("text").attr("x",-40).attr("dy",5).attr("text-anchor","start").text(d=>d.data.fullName||"(Chưa nhập)");
    }

    // ==== ZOOM ====
    let zoom=1;
    document.getElementById("zoomIn").onclick = ()=>{ zoom+=.1; document.getElementById("genealogyTree").style.transform=`scale(${zoom})`; };
    document.getElementById("zoomOut").onclick = ()=>{ zoom=Math.max(.5,zoom-.1); document.getElementById("genealogyTree").style.transform=`scale(${zoom})`; };
    ...
    async function askAI(q){
        ...
        } catch(e){ console.error(e); return "Không kết nối AI.";}
    }
