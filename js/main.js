...
fetch("data/genealogy.json")
  ...
  .then(json => {
    ...
    // Khởi tạo dropdown & cây
    refreshDropdowns();
    renderFullTree();
  })
  .catch(err => {
    console.error("❌ Lỗi load dữ liệu:", err);
    alert("Không thể load dữ liệu. Vui lòng kiểm tra data/genealogy.json");
  });
...
function drawTree(centerName = null) {
  ...
  function getChildren(nodeName) {
    return people.filter(p => p.parent === nodeName).map(c => ({ ...c, children: [] }));
  }
